  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 mt-20">
      {techData.map((data, index) => (
        <div
          className="group cursor-pointer flex items-center justify-center flex-col gap-5 hover:shadow hover:shadow-gray-800 shadow-inner shadow-black py-4 p-2 rounded-lg"
          key={index}
        >
          <Image
            src={data.logo}
            alt="logo"
            className={
              "lg:w-28 md:w-24 w-20 shadow-lg rounded-lg p-3 cursor-pointer duration-300 animate-pulse group-hover:scale-105" +
              " " +
              data.shadowColor
            }
          />
